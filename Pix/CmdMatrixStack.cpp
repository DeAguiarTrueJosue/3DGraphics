...
bool CmdPushRotationX::Execute(const std::vector<std::string>& params)
{
	...
	VariableCache* vc = VariableCache::Get();
	const float x = vc->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationX(x * MathHelper::DegToRad);
	return true;
}
...
bool CmdPushRotationY::Execute(const std::vector<std::string>& params)
{
	...
	VariableCache* vc = VariableCache::Get();
	const float y = vc->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationY(y * MathHelper::DegToRad);
	return true;
}
...
bool CmdPushRotationZ::Execute(const std::vector<std::string>& params)
{
	...
	VariableCache* vc = VariableCache::Get();
	const float z = vc->GetFloat(params[0]);
	MatrixStack::Get()->PushRotationZ(z * MathHelper::DegToRad);
	return true;
}
